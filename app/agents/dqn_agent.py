...
class DQNAgent(BaseAgent):
    def __init__(
        ...
        self.memory: deque[Tuple[State, int, float, State, bool]] = deque(
            maxlen=memory_size
        )
        ...
        self.target_network = DQNNetwork(self.state_size, self.action_size).to(
            self.device
        )
    ...
    def act(self, state: State) -> Action:
        ...
        state_tensor = (
            torch.FloatTensor(state.encode(self.env)).unsqueeze(0).to(self.device)
        )
    ...
    def _random_action(self) -> Action:
        ...
        return Action(shot_type=action[0], shot_direction=action[1])
    ...
    def replay(self) -> float:
        ...
        states = torch.FloatTensor([e[0].encode(self.env) for e in batch]).to(
            self.device
        )
        actions = torch.LongTensor([e[1] for e in batch]).to(self.device)
        rewards = torch.FloatTensor([e[2] for e in batch]).to(self.device)
        next_states = torch.FloatTensor([e[3].encode(self.env) for e in batch]).to(
            self.device
        )
