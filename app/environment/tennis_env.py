...
import random
class TennisEnv:
    def __init__(self, transition_graph: Dict[str, Dict[str, Dict[tuple, float]]], serve_first: bool = True):
        self.action_space = None  # Define action space if needed
        self.reverse_action_space = {v: k for k, v in self.action_space.items()} if self.action_space else None
        self.direction_space = {0:"1",1:"2",2:"3"}
        self.reverse_direction_space = {"1":0,"2":1,"3":2}
        self.errors = {"@", "#"}
        self.winners = {"winner"}
    ...
    def reset(self):
        ...
        # Reset other environment parameters
    ...
    def step(self, action):
        ...
        self._compute_score(self.state)
    ...
    def _choose_next_action(self, action:Action) -> Action:
        possible_next_actions = self.transition_graph.get(
            action.shot_type, {}
        ).get(
        ...
        # Sample next state
        next_shot_type, next_shot_direction = random.choices(candidates, weights=probs, k=1)[0]

        print(f"Chosen next action: ({next_shot_type}, {next_shot_direction})")

        return Action(
            shot_type=next_shot_type,
            shot_direction=next_shot_direction
        )
    ...
    def _compute_score(self, state: State) -> State:
        print("")
        # Logic to compute the next state based on the current state
        # Compute game score, set score, etc. based on errors, winners
        pass
