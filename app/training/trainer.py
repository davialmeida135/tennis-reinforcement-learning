...
class Trainer:
        ...
        with mlflow.start_run(run_name=run_name, tags=tags):
            ...
            for episode in range(episodes):
                ...
                if episode >= 99:
                    ...
                    win_rate = sum(1 for r in recent_rewards if r > 0) / len(
                        recent_rewards
                    )
                    ...
                    if avg_reward > best_avg_reward:
                        ...
                        mlflow.log_metric(
                            "best_avg_reward", best_avg_reward, step=episode
                        )
                ...
                if episode % 100 == 0:
                    avg_reward = np.mean(
                        self.training_history["episode_rewards"][-100:]
                    )
                    print(
    ...
    def _log_environment_info(self):
        ...
        mlflow.log_param(
            "stroke_types", list(self.env.stroke_space.values())[:10]
        )  # Log first 10
        mlflow.log_param("directions", self.env.direction_space)

    def _log_evaluation_results(
        self, eval_results: Dict, episode: int, prefix: str = "eval_"
    ):
